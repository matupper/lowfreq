...
entry). Built for pending events (`src/app/admin/page.tsx`): a plain
table with approve/reject buttons, gated server-side on `users.is_admin`
with a redirect for non-admins, matching Profile's gated-screen pattern.
*(Future)* extend the same page with a venue-claims section (Track B) —
...
- [x] Simple internal admin review queue for pending events (Track A
      ...
      direct URL only)
- [ ] Same admin queue extended to venue claims (Track B)
